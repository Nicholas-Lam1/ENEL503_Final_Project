...
for filename in os.listdir(image_directory):
    if filename.endswith((".jpg", ".png", ".jpeg", ".bmp")):  # Supported image formats
        ...
        final_result = cv2.add(result, result2)
        final_result = cv2.add(black_background, final_result)

        kernel = np.ones((1, 3) , np.uint8)
        final_result = cv2.erode(final_result, kernel, iterations=1)

        kernel = np.ones((1, 5) , np.uint8)
        final_result = cv2.dilate(final_result, kernel, iterations=2)
    
        kernel = np.ones((1, 5) , np.uint8)
        final_result = cv2.erode(final_result, kernel, iterations=2)
        ...
        image4_rgb = cv2.cvtColor(final_result, cv2.COLOR_BGR2RGB)
